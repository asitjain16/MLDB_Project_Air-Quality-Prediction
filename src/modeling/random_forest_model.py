...
from src.utils.constants import RANDOM_FOREST_PARAMS, RANDOM_SEED

logger = get_logger(__name__)


class RandomForestModelError(Exception):
    """Custom exception for Random Forest model errors."""
    pass
...
class RandomForestModel:
    ...
    def __init__(
        ...
        if hyperparameters is None:
            self.hyperparameters = RANDOM_FOREST_PARAMS.copy()
        else:
        ...
        logger.info(
            f"RandomForestModel initialized with hyperparameters: "
    ...
    def train(
        ...
        if X_train is None or len(X_train) == 0:
            raise RandomForestModelError("X_train cannot be None or empty")
        
        if y_train is None or len(y_train) == 0:
            raise RandomForestModelError("y_train cannot be None or empty")
        
        if len(X_train) != len(y_train):
            raise RandomForestModelError(
    ...
    def _train_with_cv(
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv_splits):
            ...
            fold_model = RandomForestRegressor(**self.hyperparameters)
    ...
    def _train_final(
        ...
        try:
            ...
            raise RandomForestModelError(f"Model training failed: {str(e)}")
    ...
    def predict(
        ...
        if self.model is None:
            raise RandomForestModelError("Model not trained. Call train() first.")
        
        if X is None or len(X) == 0:
            raise RandomForestModelError("X cannot be None or empty")
        ...
        try:
            ...
            raise RandomForestModelError(f"Prediction failed: {str(e)}")
    ...
    def predict_24h(
        ...
        if X_current is None or len(X_current) == 0:
            raise RandomForestModelError("X_current cannot be None or empty")
        ...
        try:
            ...
            raise RandomForestModelError(f"24-hour prediction failed: {str(e)}")
    ...
    def evaluate(
        ...
        try:
            ...
            raise RandomForestModelError(f"Evaluation failed: {str(e)}")
    
    def save(self, path: str) -> None:
        
        if self.model is None:
            raise RandomForestModelError("Model not trained. Cannot save.")
        
        try:
            joblib.dump(self.model, path)
            logger.info(f"Model saved to {path}")
        except Exception as e:
            raise RandomForestModelError(f"Failed to save model: {str(e)}")
    ...
    def load(self, path: str) -> None:
        ...
        Raises:
            RandomForestModelError: If load fails
        """
        try:
            self.model = joblib.load(path)
            logger.info(f"Model loaded from {path}")
        except Exception as e:
            raise RandomForestModelError(f"Failed to load model: {str(e)}")
    ...
    def get_model_info(self) -> Dict:
        ...
        Returns:
            Dictionary with model info: hyperparameters, feature_columns,
